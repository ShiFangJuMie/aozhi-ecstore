...
class b2c_ctl_admin_member extends desktop_controller{
   ...
   function _views(){
    ...
    function checkBut(){
        ...
        $this->pagedata['op_id'] = kernel::single('desktop_user')->get_id();
        $this->pagedata['memInfo'] = $membersData;

        $this->singlepage('admin/member/check.html');
    }
    //会员审核确认
    function confirm_member(){
        $this->begin('index.php?app=b2c&ctl=admin_membr&act=index');
        $obj_member = app::get('b2c')->model('members');
        if(!empty($_POST['member_id'])){
            ...
            $this->end(flase, app::get('b2c')->_('审核失败！'));
        }
    }
    //end
    function send_msg(){
