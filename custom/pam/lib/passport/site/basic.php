...
class pam_passport_site_basic
{
    ...
    public function login($userData,$vcode=false,&$msg){
        ...
        //用户登录时的账号审核状态的判断 by albert 2017.12.14
        $obj_b2c_members = app::get('b2c')->model('members');
        $member = $obj_b2c_members->getRow('check_status',array('member_id'=>$account[0]['member_id']));
        if($member['check_status'] == 1 ){
            $msg = "该账号未开通，请联系客服!";
            return false;
        }
        //end
